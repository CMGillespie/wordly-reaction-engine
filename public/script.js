// Wordly Secure Viewer Script (v18 - Background Persistence Fix)
// v18: Added silent audio loop, Media Session API, Page Visibility reconnect handler
document.addEventListener('DOMContentLoaded', () => {
  ...
  // A silent looping audio element keeps the OS audio session alive.
  // Once the user interacts (connect button), we start this loop.
  // The OS treats the page as an active media app and stops killing the WebSocket.
  const silentAudio = new Audio();
  silentAudio.src = 'silent.mp3';
  silentAudio.loop = true;
  silentAudio.volume = 0.001; // Effectively silent but not zero (zero can be optimized away)
  ...
  function setupMediaSession() {
    ...
    // Override media controls so the OS doesn't show a "pause" button
    // that would kill our silent loop
    navigator.mediaSession.setActionHandler('play', () => silentAudio.play());
    navigator.mediaSession.setActionHandler('pause', () => {}); // intentionally no-op
    navigator.mediaSession.setActionHandler('stop', () => {});
  }

  // Page Visibility handler: when user returns from background,
  // force a WebSocket reconnect check immediately
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
      ...
      // If we're in a session and the socket is dead, reconnect immediately
  ...
  // --- END v18: BACKGROUND PERSISTENCE ---
  ...
  function connect() {
    ...
    // v18: Start silent audio loop and register Media Session on connect
    // (must happen after user gesture — connect button click qualifies)
    startSilentAudio();
    setupMediaSession();
  ...
  function disconnect() {
    ...
    stopAndClearAudio();
    stopSilentAudio(); // v18: stop silent loop on deliberate disconnect
